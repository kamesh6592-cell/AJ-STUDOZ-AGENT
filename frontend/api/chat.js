...
export default async function handler(req) {
  ...
  try {
    ...
    if (!messages || !provider) {
      return new Response(JSON.stringify({ error: 'Missing required parameters: messages and provider' }), {
        status: 400,
    ...
    if (options.streaming) {
      ...
      const readable = new ReadableStream({
        async start(controller) {
          try {
            // Simulate typing effect by sending small chunks
...
async function callGroqAPI(messages, options) {
  ...
  const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      max_tokens: options.length || 2048,
